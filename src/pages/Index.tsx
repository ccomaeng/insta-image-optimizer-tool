      ...
      <div className="container max-w-xl bg-white shadow-xl rounded-lg p-6 md:p-8">
        ...
        <div className="space-y-6">
          <div>
            ...
            <Button onClick={handleUploadClick} className="w-full flex items-center justify-center gap-2 bg-sky-50 text-sky-700 hover:bg-sky-100 border border-slate-300" variant="outline">
          ...
          <div>
            ...
            <div className="relative">
              <canvas ref={canvasRef} className="bg-slate-200 border border-slate-300 rounded-md max-w-full h-auto mx-auto" style={{
              maxHeight: '400px'
            }}></canvas>
            </div>
          </div>
          
          <Button onClick={handleOptimizeAndDownload} className="w-full bg-sky-600 text-white hover:bg-sky-700 flex items-center justify-center gap-2" disabled={!originalImage || isLoading}>
            {isLoading ? <Loader className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}
            최적화 및 다운로드
          </Button>

          {statusMessage && <div className="text-center text-sm text-slate-600 mt-4">
              {statusMessage}
            </div>}
        </div>
